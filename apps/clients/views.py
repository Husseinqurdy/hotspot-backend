...
<<<<<<< HEAD
    ...
    def my_profile(self, request):
        """Client aone na abadilishe taarifa zake mwenyewe (business_name, phone)."""
        try:
            client = Client.objects.select_related('user').get(user=request.user)
        except Client.DoesNotExist:
            if request.user.is_superadmin():
                return Response({'error': 'Superadmin hana profile ya Client'}, status=404)
            return Response({'error': 'Client profile haipo'}, status=404)

        if request.method == 'GET':
            return Response(ClientSerializer(client).data)

        # PATCH — ruhusu tu fields hizi mbili, chochote kingine kinaporwa
        allowed_fields = {'business_name', 'phone'}
