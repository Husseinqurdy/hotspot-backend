...
class Package(models.Model):
    ...
    class Meta: ordering = ['price']; unique_together = ['client','price']
    def __str__(self): return f"{self.name} - TZS {self.price}"
    def duration_display(self):
        if self.duration_minutes < 60: return f"Dakika {self.duration_minutes}"
        elif self.duration_minutes < 1440: return f"Saa {self.duration_minutes//60}"
