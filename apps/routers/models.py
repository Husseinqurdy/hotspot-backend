...
class MikroTikRouter(models.Model):
    ...
    # VPN IP - inakuwa VPN IP baada ya kuunganika WireGuard
    host = models.CharField(
        max_length=100,
        help_text='VPN IP ya MikroTik (mfano: 10.0.0.2) au MikroTik Cloud DDNS'
    )
...
class MikroTikJob(models.Model):
    """
    Kazi inayosubiri kutekelezwa na MikroTik kupitia polling.
    MikroTik inauliza /api/jobs/pending/ kila dakika 1 kupitia VPN.
    """
    ...
    def __str__(self):
        return f"Job {self.id} | {self.status} | Router: {self.router.name}"
